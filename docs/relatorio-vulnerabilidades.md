...
| `docker-compose.yml` / `docker-compose.test.yml` | E | Real, risco baixo: credenciais do Postgres hardcoded no arquivo versionado (o fallback `${POSTGRES_USER:-oficina}` de uma rodada anterior ainda deixava um usuário/senha fraco por padrão no arquivo) | **Corrigido** - fallback trocado pelo operador `:?` do Compose (`${POSTGRES_USER:?defina POSTGRES_USER no .env}` etc.), que falha explicitamente no `docker compose up`/`config` se a variável não estiver definida, em vez de usar uma senha fraca por padrão - mesmo princípio de "nada tem default" já aplicado em `app/shared/settings.py` |
| `app/application/usuario/use_cases/autenticar_usuario.py` | E | Falso positivo: `_HASH_SEM_CORRESPONDENCIA` era um hash bcrypt fixo no código, no formato exato de uma credencial real (mitigação de timing attack no login, nunca corresponde a senha real) | **Corrigido** - hash passou a ser gerado em runtime a partir de um valor aleatório (via `PasswordHasherProtocol`, injetado a partir da composição em `auth/dependencies.py`), eliminando a string estática que disparava o alerta, sem alterar a mitigação de timing attack |
